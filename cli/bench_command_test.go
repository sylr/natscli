...
func TestBenchDurationWithoutMsgsFlag(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			configureBenchCommand(root)

			cmd, rest, err := root.Find(tc.args)
			if err != nil {
				t.Fatalf("expected %v to resolve to a command, got: %v", tc.args, err)
			}
			...
			if err := cmd.ValidateArgs(cmd.Flags().Args()); err != nil {
				t.Fatalf("expected %v to pass argument validation, got: %v", tc.args, err)
			}
		})
	}
}
